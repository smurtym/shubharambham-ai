...
- [X] T005 [US1] Replace stub structs in `astro-wasm/src/data/mod.rs` with complete definitions — `CityRecord { city_id: u32, canonical_name: &'static str, timezone: &'static str, translations: &'static [(&'static str, TranslationEntry)] }` and `TranslationEntry { city_name: &'static str, region1: &'static str, region2: &'static str, region1_order: u16, region2_order: u16 }`
- [X] T006 [US1] Add `CityResponse`, `CityListResponse`, and `decode_city_id` to `astro-wasm/src/data/mod.rs`:
  - `pub fn decode_city_id(city_id: u32) -> (f64, f64)` — decode 15 base-4 digits (bit-interleaved, `q % 4` per digit) into `tile_x`/`tile_y`; compute `lng = (tile_x as f64 + 0.5) / 32768.0 * 360.0 - 180.0`; compute `lat` via `n = PI * (1.0 - 2.0 * (tile_y as f64 + 0.5) / 32768.0)` then `n.sinh().atan() * 180.0 / PI`; returns `(lat, lng)` in decimal degrees
  - `CityResponse` derives `serde::Serialize` with `#[serde(rename_all = "camelCase")]`; fields: `lang: String`, `city_id: u32`, `time_zone: String`, `canonical_name: String`, `city_name: String`, `region1: String`, `region2: String`, `lat: f64`, `lng: f64`; mark `region1_order: u16` and `region2_order: u16` with `#[serde(skip)]`
